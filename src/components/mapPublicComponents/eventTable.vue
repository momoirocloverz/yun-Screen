<template>
  ...
  >
    ...
    />
    <AuditCase
      :isShow="isShowAuditCase"
      :row="row"
      @close="auditCaseClose"
      @update="get_xccz_list"
    />
    <DispatchCheck
      :isShow="isShowDispatchCheck"
      :row="row"
      :type="dispatchType"
      @close="dispatchCheckClose"
      @update="get_xccz_list"
    />
    <CheckRoutine
      :isShow="isShowCheckRoutine"
      :row="row"
      @close="checkRoutineClose"
      @update="get_xccz_list"
    />
  </div>
</template>
...
export default {
  components: { TableC, TaskProcess, AuditCase, DispatchCheck, CheckRoutine },
  props: {
    isShow: {
      type: Boolean,
      default: false,
  ...
  data() {
    return {
      ...
      // 方案审核
      isShowAuditCase: false,
      // 下派核查
      isShowDispatchCheck: false,
      dispatchType: '',
      // 例行检查
      isShowCheckRoutine: false,
    }
  },
  mounted() {
  ...
  methods: {
    ...
    tableBtnClick(row, btnName) {
      ...
      if (btnName === '查看') {
        ...
        this.logo += '1'
      } else if (btnName === '方案审核') {
        console.log('方案审核', row)
        if (row.zgfaStatus == '待审核') {
          this.isShowAuditCase = true
        } else if (row.zgfaStatus == '未提交') {
          this.$message.warning('方案尚未提交')
        } else if (row.zgfaStatus == '已通过') {
          this.$message.warning('方案已审核通过')
        }
      } else if (btnName === '下派核查') {
        console.log('下派核查', row)
        if (
          row.rectifyStatus === '完成整改任务' ||
          row.checkStatus === '未通过'
        ) {
          this.dispatchType = 'add'
          this.isShowDispatchCheck = true
        } else if (row.checkStatus === '已通过') {
          this.$message.warning('核查已通过')
        } else if (row.rectifyStatus === '下发核查任务') {
          this.$message.warning('已下发核查任务')
        } else {
          this.$message.warning('镇街未完成整改任务')
        }
      } else if (btnName === '完成核查') {
        this.isShowDispatchCheck = true
        this.dispatchType = 'confirmFinish'
      } else if (btnName === '核查结果') {
        console.log('核查结果', row)
        if (row.rectifyStatus === '提交检查结果') {
          this.isShowCheckRoutine = true
        } else {
          this.$message.warning('巡查结果尚未提交')
        }
      }
    },
    auditCaseClose() {
      this.isShowAuditCase = false
    },
    dispatchCheckClose() {
      this.isShowDispatchCheck = false
    },
    checkRoutineClose() {
      this.isShowCheckRoutine = false
    },
    // 获取巡查处置列表
    get_xccz_list() {
      let params = {
      ...
      xccz(params).then((res) => {
        ...
        this.tableData = res.data.data.list.map((item) => {
          ...
          if (item.checkType === '安全整改') {
            if (item.rectifyStatus === '下发核查任务') {
              item.btnArr = ['完成核查']
            } else {
              item.btnArr = ['方案审核', '下派核查']
            }
