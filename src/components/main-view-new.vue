<template>
  <div class="main-view">
    ...
    <router-view v-if="isShow_center && !isShow_model" name="center"></router-view>
    <!-- <map3d></map3d> -->
    <div class="top" v-show="isShow_top">
    ...
    <div class="event-track" v-if="!isShow_center">
    ...
    <!-- 展示3d模型 -->
    <div class="back" v-show="isShow_model" @click="model_back"></div>
    <div class="box-3d" v-if="false">
      <!-- <Model3D /> -->
    </div>
    <iframe
      ref="iframe"
      v-if="needLoadModel"
      :style="{ opacity: isShow_model ? 1 : 0 }"
      style="width: 100%; height: 100%; border: none"
      src="http://www.thingjs.com/pp/b0fcefbac5764e0fca3b6ed8"
    ></iframe>
...
export default {
  ...
  watch: {
    currentRouteName(val) {
      ...
      this.isShow_modelVideo = false;
      //   if (val !== '/wbd') {
      //     this.isShow_model = false;
      //   }
      if (val == '/wbd' || val == '/yxzsDetails') {
      } else {
        this.isShow_model = false;
      }
      // if (val !== '/zbz' && val !== '/zbz2') {
      //   this.isShow_center = true;
      //   this.isShow_top = true;
      // }
      this.isShow_center = true;
      this.isShow_top = true;
    },
  ...
  created() {
    ...
    eventBus.$on('isShowCenterAndTop', (boolean, logo) => {
      this.isShow_center = boolean;
      this.isShow_top = boolean;
      if (boolean) {
        // CesiumPop.showDivPop(viewer);
      } else {
        if (logo == 'onlyPreview') {
    ...
    eventBus.$on('showModel', () => {
      this.isShow_model = true;
    });
    eventBus.$on('hideModel', () => {
      this.isShow_model = false;
    });
    eventBus.$on('showExistModel', (show) => {
      this.needLoadModel = show;
    });
  ...
  methods: {
    ...
    model_back() {
      if (this.$route.name == 'yxzsDetails') {
        this.$router.replace({ name: 'yxzs' });
      } else {
        this.$router.back();
      }
      this.isShow_model = false
