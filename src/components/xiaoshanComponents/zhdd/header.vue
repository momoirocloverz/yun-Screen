<template>
  <div id="header" class="header">
    <div class="headerImg">
      ...
      <span style="font-size: 32px; font-weight: 900; flex: 1; text-align: center; margin-bottom: 20px; font-family: 'Trebuchet MS'; cursor: pointer" @click="routerJump('/jsc')">{{ headTitle }}</span>
      <div class="date">
...
export default {
  ...
  methods: {
    ...
    getTreeDataChildList(treeData, p_val) {
      if (this.childNetInfo instanceof Array && this.childNetInfo.length > 0) return;
      for (let i in treeData) {
        if (treeData[i].newVal === p_val) {
          ...
          this.currentNetPoi_1 = treeData[i].netPoi_1 ? treeData[i].netPoi_1 : '';
          this.currentNetPoi_2 = treeData[i].netPoi_2 ? treeData[i].netPoi_2 : '';
          this.currentNetName = treeData[i].netName;
          this.currentRegionColor = treeData[i].netColor;
          this.parentRegion = p_val.split('_')[0] * 1 - 1 + '_' + treeData[i].netPid;
        } else {
    ...
    currentRegionChange(val) {
      ...
      if (currentRegionPid.indexOf('-') < 0 && currentRegionId != '-2' && currentRegionId != '-3') {
        // 行政区划
        currentRegionType = '1';
