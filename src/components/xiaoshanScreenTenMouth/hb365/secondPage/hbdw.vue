<template>
  <div class="conhbdwtainer" v-if="isShow">
    <div class="center-container" :style="{ opacity: maskOpacity }">
      <div class="head">护保队伍详情</div>
    ...
    <HBYDetail :isShow="isShow_hbyDetail" @close="close_hbyDetail" />
  </div>
</template>
<script>
import { wbd_list2, szzj } from '../../../../api/xsApi.js';
import { get_hbdw_detail } from '../../../../api/xsApi3';
import Table from './component/table';
import HBYDetail from './hbyDetail';
    ...
    return {
      ...
      tableColumns: [
        ...
        },
        {
          ...
          flex: 1,
          format: (row) => {
            if (row.roleName && row.roleName == '文保员') {
              return '护保员';
            } else {
              return row.roleName;
            }
        ...
        },
        {
          ...
          props: 'jd',
          format: (row) => {
            let temp = ['戴村镇', '河上镇', '楼塔镇', '进化镇', '党湾镇', '蜀山街道', '新塘街道', '南阳街道', '瓜沥镇', '临浦镇'];
            if (row.id) {
              return row.id;
            } else {
              return temp[+row.phone[row.phone.length - 1]];
            }
          },
        },
        {
  ...
  methods: {
    ...
    getHBDW_detail() {
      let params = {
        ...
        rylx: this.searchParams.peopleType && this.searchParams.peopleType == '护保员' ? '文保员' : this.searchParams.peopleType,
        wbdw: this.searchParams.pointName,
        xm: this.searchParams.name,
      };
      get_hbdw_detail(params).then((res) => {
