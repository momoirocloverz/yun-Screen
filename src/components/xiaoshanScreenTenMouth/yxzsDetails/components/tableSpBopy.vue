  ...
  <div class="box-table" :style="{ 'font-size': fontSize ? fontSize : '16px' }">
    <div class="head">
      ...
      <span
        ...
        >{{ value.name }}</span
      >
      <span v-if="optionList && optionList.length > 0" :style="{ flex: optionList[0].flex ? optionList[0].flex : 1 }">操作</span>
      <span v-if="hasOtherInfo" :style="{ flex: hasOtherInfo.flex }"></span>
    </div>
    <div class="tableBodyCon">
      <div class="tableItem" v-for="(item, index) in tableData" :key="item.id">
        <div
          class="outerFlex"
          :style="{
            flex: 2,
        ...
        >
          <span class="ellSelf" :title="item.type">
            {{ item.type }}
          </span>
        </div>
        <!-- <span
        ...
        <div
          ...
          class="resetColumHeight"
        >
          <div v-for="(subItem, subIndex) in item.itemArray" :key="subIndex" class="flexInner">
            <span
              :style="{
                flex: 7,
              }"
              class="ellSelf withBorder"
            >
              {{ subItem.rule }}
            </span>
            <span
...
.flexInner {
  ...
  height: auto;
  box-sizing: border-box;
  border-bottom: 1px solid #4e5f79;
  &:last-child {
    border-bottom: none;
  }
}
