<template>
  <div class="yxzsDetailSmartModalCon" v-if="isShow" :style="{opacity:maskOpacity}">
    <div class="centerCon">
...
import { get_ZF_list_data } from '../../../../api/xsApi3.js';

import Detail from './zhiFangDetail'
export default {
  props: {
    isShow: {
      type: Boolean,
  ...
  methods: {
    ...
    getList() {
      let params={
        pointName:this.$route.query.id?this.$route.query.id:this.searchParams.pointId,
        currentPage:this.currentPage,
        pageSize:this.pageSize,
        type:this.currentType
      }
      get_ZF_list_data(params).then(res=>{
        console.log('智防列表',res.data.data)
