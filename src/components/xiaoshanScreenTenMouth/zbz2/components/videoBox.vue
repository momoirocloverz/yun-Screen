<template>
  <div :id="`player_${index}_${videoId}`" :style="{ width: width, height: height }"></div>
</template>
<script>
...
export default {
  ...
  watch: {
    videoId(val) {
      getHikVideoUrl(val).then((res) => {
        ...
        this.player.JS_Play(res.data.data.url, { playURL: res.data.data.url, mode: 0 }, index).then(
          () => {
            console.log('realplay success watch');
  ...
  beforeDestroy() {
    // console.log('摄像机销毁了');
    this.player.JS_StopRealPlayAll().then((res) => {
      //   console.log('res', res);
    });
  },
  methods: {
...
<style lang="less">
:not(:root):fullscreen .play-window {
