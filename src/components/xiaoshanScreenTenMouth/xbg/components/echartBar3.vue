...
export default {
  ...
  methods: {
    initchart() {
      ...
      this.legendSelected = ['国保', '省保', '市保'];
      mychart.getZr().off('click');
      mychart.getZr().on('click', (params) => {
        const pointInPixel = [params.offsetX, params.offsetY];
        if (mychart.containPixel('grid', pointInPixel)) {
          let xIndex = mychart.convertFromPixel({ seriesIndex: 0 }, [params.offsetX, params.offsetY])[0];
          //   console.log('grid', xIndex);
          //   console.log('chartData_1', this.chartData_1[xIndex].name);
          let ranksTemp = [];
          if (this.$route.name == 'xbgCopy') {
            this.legendSelected.find((ele) => {
              if (ele == '国保') {
                ranksTemp.push({
                  name: '国保单位',
                  id: '14da5819da904c4ab1d9d94a961ca27a',
            ...
            let idArray = ranksTemp.map((ele) => {
              return ele.id;
            });
            let idStr = idArray.join(',');
            this.$store.commit('setXbgCopyRankDisturbLevel', {
              level: idStr,
              area: this.chartData_1[xIndex].name,
            });
          }
        }
      });
      mychart.on('legendselectchanged', (params) => {
        let res1 = params.selected['国保'];
        let res2 = params.selected['省保'];
        let res3 = params.selected['市保'];
        let empty = [];
        if (res1) {
          empty.push('国保');
        }
