  ...
  <div class="box-table" :style="{ 'font-size': fontSize ? fontSize : '16px' }">
    <div class="head">
      ...
      <span
        ...
        >{{ value.name }}</span
      >
      <span v-if="optionList && optionList.length > 0" :style="{ flex: optionList[0].flex ? optionList[0].flex : 1, color: 'rgb(228, 240, 255)' }">操作</span>
    ...
    <div class="body">
      <div :class="['row', isStripe ? 'row_stripe' : '', isSpecal ? 'row_specal' : '']" v-for="(item, index) in tableData" :key="item.id" @click="rowClick(item)">
        ...
        >
          ...
          <i class="ellSelf" :title="item[val.props] ? item[val.props] : ''">
