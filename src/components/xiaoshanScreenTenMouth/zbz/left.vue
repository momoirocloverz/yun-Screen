<template>
  <div class="container">
    <div class="v1">
      <Box :height="'335px'" :bgSrc="require('../../../assets/imgBoxBg/zbz-box1-bg.png')">
        <div class="part-1">
          <Title :text="'AI感知'" @textClick="aigz_title_click">
            <div class="detailBtn" @click="aigz_Click">详情<i class="el-icon-arrow-right"></i></div>
          </Title>
          <div class="chart-box">
            <div class="chart">
              <LeftChartV1 :color="'#37C0D3'" :title="'设备覆盖率'" :chartData="rate_tj[0]" :max="100" :logo="logo_2" />
            </div>
            <div class="chart">
              <LeftChartV1 :color="'#F0D28C'" :chartData="rate_tj[1]" :title="'设备在线率'" :logo="logo_2" />
            </div>
          </div>
          <div class="tj">
            <div class="item" v-for="(item, index) in tj_data" :key="index" @click="deviceTj_click(item)">
              <div class="img">
                <div class="chart">
                  <LeftChartMin :chartData="item.rate" />
                  <img :src="item.imgSrc" alt="" width="auto" height="auto" />
                </div>
                <span>{{ item.name }}</span>
              </div>
              <div class="info">
      ...
      <Box :height="'580px'" :bgSrc="require('../../../assets/imgBoxBg/zbz-box2-bg.png')">
        <div class="part-2">
          <Title :text="'AI预警'">
            <div class="btn-box">
              <div style="width: 80px; height: 25px">
                <el-select v-model="currentSelect" size="mini" placeholder="" style="width: 80px" @change="aiyj_type_change">
                  <el-option v-for="item in selectOPtion" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                </el-select>
              </div>
              <div class="detailBtn" @click="aiyj_Click">详情<i class="el-icon-arrow-right"></i></div>
            </div>
          </Title>
          <div class="list globalFireFoxScrollBar">
              ...
              <div v-if="index == currentIndex" class="time-line">
                <el-timeline>
                  <el-timeline-item timestamp="" placement="top">
                    <div class="content">
                      <div class="system">
                        <span>{{ item.timeLine[0].title }}</span>
                        <span>{{ item.timeLine[0].time }}</span>
                      </div>
                      <span class="info"> {{ item.timeLine[0].content }} </span>
                    </div>
                  </el-timeline-item>
                  <el-timeline-item timestamp="" placement="top">
                    <div class="content">
                      <div class="qzzx">
                        <span>{{ item.timeLine[1].title }}</span>
                        <span>{{ item.timeLine[1].time }}</span>
                      </div>
                      <span class="info"> {{ item.timeLine[1].content }} </span>
                    </div>
                  </el-timeline-item>
                  <el-timeline-item timestamp="" placement="top">
                    <div class="content">
                      <div class="street">
                        <span>{{ item.timeLine[2].title }}</span>
                        <span>{{ item.timeLine[2].time }}</span>
                      </div>
                      <span class="info"> {{ item.timeLine[2].content }} </span>
                    </div>
                  </el-timeline-item>
                </el-timeline>
              </div>
          ...
          <div class="chart">
            <LineChart :chartData_1="lineData_1" :chartData_2="lineData_2" :chartData_3="lineData_3" :chartData_4="lineData_4" :logo="logo" />
          </div>
        </div>
      </Box>
    </div>
...
import { get_aigz_data, get_aiyj_data, get_aizb_data, get_skjc_data, selectAiEventPage, selectAiStatistics, get_aiyj_detail } from '../../../api/xsApi3';
...
export default {
  ...
  mounted() {
    ...
    // this.fetchAiEventPage();
  },
  methods: {
    formatTime1(val) {
      let currentTime = dayjs(val).format('YY/MM/DD HH:mm:ss');
      return currentTime;
    },
    fetchAiYjList() {
      let params = {
        currentPage: 1,
        pageSize: 20,
        sblx: this.currentSelect == '0' ? undefined : this.currentSelect,
      };
        ...
        if (res && res.data && res.data.code && res.data.code == 200) {
          ...
          if (shorter) {
            shorter.list.forEach((ele) => {
              ele.foundTime = ele.foundTime ? this.formatTime1(ele.foundTime) : null;
              ele.checkCreateTime = ele.checkCreateTime ? this.formatTime1(ele.checkCreateTime) : null;
              ele.endTime = ele.endTime ? this.formatTime1(ele.endTime) : null;
              ele.task = '智能感知预警';
              ele.timeLine = [
                {
                  time: ele.foundTime,
                ...
                {
                  time: ele.checkCreateTime,
                  title: '文保员',
                  content: ele.userName,
                },
                {
                  time: ele.endTime ? ele.endTime : ele.checkCreateTime,
                  content: ele.checkContent,
                  title: '处置结果',
                },
              ];
            });
            this.listData = shorter.list;
          }
        }
      });
    },
    fetchAiEventPage() {
      ...
      selectAiEventPage(params).then((res) => {
        console.log(res);
      });
    },
    fetchAiStatistics() {
      selectAiStatistics({}).then((res) => {
        console.log(res);
      });
    },
    jmzkBoxHidden() {
    ...
    getAIGZ_data() {
      setTimeout(() => {
        ...
        this.rate_tj[1] = 100;
        this.logo_2 += '1';
        this.tj_data[0].num = 327;
        this.tj_data[0].rate = 50;
        this.tj_data[1].num = 32;
        this.tj_data[1].rate = 12;
        this.tj_data[2].num = 187;
        this.tj_data[2].rate = 72;
        this.tj_data[3].num = 32;
        this.tj_data[3].rate = 17;
      }, 800);
    ...
    aigz_title_click() {
      ...
      if (this.$route.name == 'zbz') {
        eventBus.$emit('devicePoi', arr);
      } else if (this.$route.name == 'zbzCopy') {
        this.$store.commit('setZbzCopyAIType', arr);
      }
    },
    aigz_Click() {
    ...
    deviceTj_click(item) {
      ...
      switch (item.name) {
        case '烟感':
          arr = ZBZStaticData.filter((ele) => {
            return ele.type == 'yangan';
          });
          break;
        case '水压':
          arr = ZBZStaticData.filter((ele) => {
            return ele.type == 'shuiya';
          });
          break;
        case '监控':
          arr = ZBZStaticData.filter((ele) => {
            return ele.type == 'jiankong';
          });
          break;
        case '电压':
          arr = ZBZStaticData.filter((ele) => {
            return ele.type == 'dianya';
          });
          break;
        default:
          break;
