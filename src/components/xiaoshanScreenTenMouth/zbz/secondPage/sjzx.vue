...
let staticData_sjzx = [
  ...
  },
  {
    ...
    roleName: '护保员',
    createTime: '2023-01-12',
    content: '东蓍草庵消防水龙头前设置铁杆，妨碍消防接管出水',
    status: '处理中',
  },
  {
    pointName: '许家南大房',
  ...
  methods: {
    ...
    getSJZX_detail() {
      ...
      get_sjzx_detail(params).then((res) => {
        if (this.currentPage == 1) {
          this.tableData = staticData_sjzx.concat(res.data.data.list);
        } else {
          this.tableData = res.data.data.list;
        }
        this.total = res.data.data.total * 1 + staticData_sjzx.length;
      });
    },
    close() {
      this.$emit('close');
