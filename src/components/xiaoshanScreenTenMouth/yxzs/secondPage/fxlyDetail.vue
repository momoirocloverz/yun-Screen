      ...
      <div class="body-box">
        <div class="head-info">
          ...
          <div class="info">
            ...
            <div class="bottom">
              <div class="col">
                <span class="key">检查人员：</span>
                <span class="value">{{ detailInfo.nickName || '田欢乐' }}</span>
              </div>
              <div class="col">
                <span class="key">检查时长：</span>
                <span class="value">暂无</span>
              </div>
              <div class="col">
                <span class="key">上报时间：</span>
                <span class="value">{{ detailInfo.bodyTime || '2023-03-12 12:56:15' }}</span>
              </div>
            </div>
          </div>
        </div>
        <div class="before-info">
          <div class="part part-1">
            <div class="title">检查名称：</div>
            <div class="content">{{ detailInfo.checkName || '务本堂例行检查' }}</div>
          </div>
          <div class="part part-2">
            <div class="title">工作照片：</div>
            <div class="content" v-if="workImgList.length > 0">
              <!-- <img v-for="(item,index) in workImgList" :key="index" :src="item.img"> -->
          ...
          <div class="part part-3">
            <div class="title">巡查问题：</div>
            <div class="content" v-if="checkList.length > 0">
              <div class="item" v-for="(item, index) in checkList" :key="index">
                <div class="title">{{ index + 1 }}、{{ item.checkNameType }}</div>
                <div class="detail" v-if="item.otherDescriptions.length > 0">{{ item.otherDescriptions }}</div>
                <div class="img">
                  <!-- <img v-for="(val,i) in item.imgList" :key="i" :src="val.img"> -->
                  <template v-for="(sub, subIndex) in item.imgList">
                    <img @click="() => showImg(subIndex, item.imgList)" :src="sub.img" class="thumbImg" :key="subIndex" />
                  </template>
                  <viewer :images="item.imgList" :options="options" @inited="inited" class="viewer" ref="viewer2" v-show="false">
                    <img v-for="(src, index) in item.imgList" :src="src.img" :key="index" />
                  </viewer>
                </div>
              </div>
            </div>
            <div class="content" v-else>暂无问题</div>
          </div>
        ...
        <div class="after-info" v-if="detailInfo.jdIsRunState !== 4">
          <div class="part part-1" v-if="detailInfo.jdIsRunState !== 99">
            <div class="title">处置情况：</div>
            <div class="content">
              <div class="it">
                <div class="key">处置事项：</div>
                <div class="content-sub">{{ detailInfo.baseCheckEnds && detailInfo.baseCheckEnds.length ? detailInfo.baseCheckEnds[0].endContent : '暂无' }}</div>
              </div>
              <div class="it it-2">
                <div class="key">处置照片：</div>
                <div class="content-sub" v-if="handleImgList.length > 0">
                  <template v-for="(sub, subIndex) in handleImgList">
                    <img @click="() => showImg(subIndex, handleImgList)" :src="sub.img" class="thumbImg" :key="subIndex" />
                  </template>
                  <viewer :images="handleImgList" :options="options" @inited="inited" class="viewer" ref="viewer2" v-show="false">
                    <img v-for="(src, index) in handleImgList" :src="src.img" :key="index" />
                  </viewer>
                </div>
                <div class="content-sub" v-else>暂无图片</div>
              </div>
              <div class="it">
                <div class="key">处置人：</div>
                <div class="content-sub">{{ detailInfo.baseCheckEnds && detailInfo.baseCheckEnds.length ? detailInfo.baseCheckEnds[0].endUserName : '暂无' }}</div>
                <div class="key">处置时间：</div>
                <div class="content-sub">{{ detailInfo.baseCheckEnds && detailInfo.baseCheckEnds.length ? detailInfo.baseCheckEnds[0].startTime : '暂无' }}</div>
              </div>
            </div>
          </div>
          <div class="part part-2">
          ...
          if (shorter) {
            ...
            this.checkList =
              ...
              this.detailInfo.details
                .map((item) => {
                  if (item.isCheck == 0 || item.isCheck == 10 || !item.checkNameType) {
                    let str = item.storageId;
                    item.imgList = str ? JSON.parse(str) : [];
                    return item;
                  }
                })
                .filter((val) => val);
            this.checkList.length > 0 &&
              this.checkList.map((item, index) => {
                if (!item.checkNameType) {
                  ...
                  this.workImgList = str ? JSON.parse(str) : [];
                }
              });
            if (this.detailInfo && this.detailInfo.baseCheckEnds && this.detailInfo.baseCheckEnds.length) {
              if (this.detailInfo.baseCheckEnds[0].startImages) {
                this.handleImgList = JSON.parse(this.detailInfo.baseCheckEnds[0].startImages);
              }
            }
          }
        }
